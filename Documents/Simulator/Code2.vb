Module Code2
    ...
    Private Sub OUTPUT(ByRef register As Byte, ByVal address As Byte)
        ...
        Select Case address
            ...
            Case &H70
                If register = 1 Then
                    Ram.Write(2, RAM_WADDR0, RAM_WADDR1, RAM_DIN)
                End If
            Case LED_port
    ...
    Private Sub INPUT(ByRef register As Byte, ByVal address As Byte)
        ...
        Select Case address
            Case &H76
                register = Ram.Read(2, RAM_RADDR0, RAM_RADDR1)
            Case rx_uart_port
...
wait:
        _LOAD(s0, &HFF) ' това трбява да е същия вид чакане
        STORE(s0, RAMaddLSB)
        _LOAD(s0, 3)
        STORE(s0, RAMaddMSB)
        Call readRAM()
...
loopk:
loopWait:
        _LOAD(s0, &HFF) ' very beginning, set HINT2 to 0
        STORE(s0, RAMaddLSB)
        _LOAD(s0, 3)
        STORE(s0, RAMaddMSB)
        Call readRAM()
...
loopMult2:
        ...
        ADDCY(sE, 0)
        __SUB(sC, 1)
        If Not ZERO Then GoTo loopMult2 '(NZ, loopMult2)
        __ADD(s0, sD)
        ADDCY(s1, sE)
        STORE(s0, RAMaddLSB)
        STORE(s1, RAMaddMSB)
...
OK:
        ...
        __SUB(sA, 1) ' j
        If ZERO Then GoTo loopj_end 'JUMP(Z, loopj_end) Then
        __SUB(sA, 1)
...
loopi_end:
        _LOAD(s0, &HFF) 'end of k iteration waiting for HINT2 to be 0
        STORE(s0, RAMaddLSB)
        _LOAD(s0, 3)
        STORE(s0, RAMaddMSB)
        _LOAD(s0, 0)
        STORE(s0, RAMwriteDATA)
        Call writeRAM() ' set HINT2 to 0
        __SUB(s5, 1)
