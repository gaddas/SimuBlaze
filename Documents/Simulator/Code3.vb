Module Code3
    ...
    Private Sub OUTPUT(ByRef register As Byte, ByVal address As Byte)
        ...
        Select Case address
            ...
            Case &H70
                If register = 1 Then
                    Ram.Write(3, RAM_WADDR0, RAM_WADDR1, RAM_DIN)
                End If
            Case LED_port
    ...
    Private Sub INPUT(ByRef register As Byte, ByVal address As Byte)
        ...
        Select Case address
            Case &H76
                register = Ram.Read(3, RAM_RADDR0, RAM_RADDR1)
            Case rx_uart_port
...
wait:
        _LOAD(s0, &HFE) ' това трбява да е същия вид чакане
        STORE(s0, RAMaddLSB)
        _LOAD(s0, 3)
        STORE(s0, RAMaddMSB)
        Call readRAM()
...
loopk:
loopWait:
        _LOAD(s0, &HFE) ' very beginning, set HINT3 to 0
        STORE(s0, RAMaddLSB)
        _LOAD(s0, 3)
        STORE(s0, RAMaddMSB)
        Call readRAM()
...
loopi_end:
        _LOAD(s0, &HFE) 'end of k iteration waiting for HINT3 to be 0
        STORE(s0, RAMaddLSB)
        _LOAD(s0, 3)
        STORE(s0, RAMaddMSB)
        _LOAD(s0, 0)
        STORE(s0, RAMwriteDATA)
        Call writeRAM() ' set HINT3 to 0
        __SUB(s5, 1)
