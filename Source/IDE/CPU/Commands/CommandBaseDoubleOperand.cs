...
{
    class CommandBaseDoubleOperand : CommandBase
    {
        ...
        public override void Assemble(string[] lines, TheCpu cpu, TheAssembler assembler)
        {
            ...
            {
                // CMD sX, sY
                this.AssembleV1(lines[1].Remove(lines[1].Length - 1), lines[2], assembler);
            }
            else if (cpu.ValidLiteral(lines[2]))
            {
                // CMD sX, kk
                this.AssembleV2(lines[1].Remove(lines[1].Length - 1), lines[2], assembler);
            }
            else
            {
                throw new ArgumentException("Невалидна инструкция.");
